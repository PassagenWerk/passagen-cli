...
- `SQLite`、`SQLAlchemy 2.x` 和 `Alembic`：持久化与迁移
- `httpx`：GROBID 和 Semantic Scholar 请求
...
- 递归或非递归扫描指定目录中的 PDF。
- 流式计算 SHA-256，避免将整个 PDF 载入内存。
- 登记新文件并跳过已有文件哈希。
- 保存原文件名、导入路径、文件大小和导入时间。
- 实现论文列表、详情和状态过滤。
...
- 实现 Semantic Scholar Academic Graph API 客户端。
- 使用 DOI 或 arXiv ID 精确查询元数据。
- 保存 title、authors、year、venue、source URL 和字段来源。
- Semantic Scholar 未命中或不可用时使用 PDF 元数据继续处理。
- 当 DOI 或 arXiv ID 指向已有论文时执行安全合并。
- 保留同一论文不同 PDF 文件或版本的导入记录。
...
- Semantic Scholar 请求使用 DOI/arXiv ID，不通过模糊标题自动合并。
- API 超时、限流和未命中不会丢失已有解析结果。
- 合并记录后，产物和处理历史仍关联到正确的 `paper_id`。
- 使用 HTTP mock 覆盖成功、未命中、限流和服务错误。
...
- 在归档目录中保存 PDF、解析结果、摘要和 outline。
- 增加 GROBID、Semantic Scholar 和 LLM 的超时及指数退避。
...
- 对同一目录再次运行不会重复调用 Semantic Scholar 或 LLM。
...
- 缺少 GROBID、Semantic Scholar key 或 LLM key 时错误信息明确。
- 用户数据目录可以整体复制并在另一环境中继续使用。
- 所有自动化测试、静态检查和数据库迁移检查通过。
...
- GROBID、Semantic Scholar 和 LLM 的 mock HTTP 交互。
